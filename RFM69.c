...
bool rfm69_initialize(char freqBand, char nodeID, char networkID)
...
//    /* 0x04 */ { REG_BITRATELSB, 0x6b},//0xA0},//RF_BITRATELSB_55555},
   ...
   /* 0x03 */ { REG_BITRATEMSB, RF_BITRATEMSB_55555}, // default: 4.8 KBPS
    ...
    /* 0x08 */ { REG_FRFMID, (char) (freqBand==RF69_315MHZ ? RF_FRFMID_315 : (freqBand==RF69_433MHZ ? RF_FRFMID_433 : (freqBand==RF69_868MHZ ? RF_FRFMID_868 : RF_FRFMID_915))) },

    /* 0x09 */ { REG_FRFLSB, (char) (freqBand==RF69_315MHZ ? RF_FRFLSB_315 : (freqBand==RF69_433MHZ ? RF_FRFLSB_433 : (freqBand==RF69_868MHZ ? RF_FRFLSB_868 : RF_FRFLSB_915))) },
...
//  do writeReg(REG_SYNCVALUE1, 0x55); while (readReg(REG_SYNCVALUE1) != 0x55 && millis()-start < timeout);
  ...
  for (i = 0; CONFIG[i][0] != 255; i++){writeReg(CONFIG[i][0],CONFIG[i][1]);}//writeReg(CONFIG[i][0], CONFIG[i][1]);}
...

{
  ...
  return RF69_FSTEP * (((unsigned long) readReg(REG_FRFMSB) << 16) + ((unsigned long) readReg(REG_FRFMID) << 8) + readReg(REG_FRFLSB));
...
void encrypt(const char* key) {
...

  {
    ...
    writeMultipleReg(REG_AESKEY1,(char *)key,16);
...
{
  static char curr_mode = 0x0;

  if (newMode == curr_mode)
...
//  }
  ...
  void setPowerLevel(char powerLevel)
  ...
  bool sendWithRetry(char toAddress, const void* buffer, char bufferSize, char retries, char retryWaitTime) {
...
*/
    ...
    transfer_buf[0]= bufferSize + 3;
    transfer_buf[1]= toAddress;
    transfer_buf[2]= _address;
    transfer_buf[3]= CTLbyte;

    for (i = 0; i < bufferSize; i++){transfer_buf[i+4] = ((char*) buffer)[i];}

    writeMultipleReg(REG_FIFO,transfer_buf,bufferSize+4);
...
//      setMode(RF69_MODE_STANDBY);
      ...
      send_addr(REG_FIFO & 0x7F);

      PAYLOADLEN = read_single();

      PAYLOADLEN = PAYLOADLEN > 66 ? 66 : PAYLOADLEN; // precaution

      TARGETID = read_single();
